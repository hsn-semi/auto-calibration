...
void adjust_LED(double *I, double current_power, double previous_power)
{
    const double tolerance = 1e-3; // Set the tolerance value
    double difference = fabs(current_power - previous_power);

    // Check if the power difference is small enough
    if (difference <= tolerance) {
        // If within tolerance, we can return
        return;
    }

    // Adjust the current based on the power difference
    if (current_power > previous_power) {
        *I -= 0.1; // Decrease current
    } else {
        *I += 0.1; // Increase current
    }

    // Ensure current stays within bounds (45 < I_current < 650)
    if (*I < 45 || *I > 650) {
        printf("Current out of range, stopping process\n");
...
int my_main() {
    ...
    double I = 50.0, U = 0, F = 0;  // Initialize current, voltage, and photocurrent
    double previous_power = 0;  // Track the previous power
    double current_power;

    unsigned char port = 0; // port for setting the current

    // Try block to simulate UART communication (replace this with actual serial communication logic)
//    while (1) {  // Infinite loop to simulate repeated checks
    for (int i=0; i<10; i++){
        // Simulate reading from UART (replace this part with actual UART reading)
        // For now, we'll just assign some values for testing.
        // Read and decode I, U, F (replace with actual UART reading logic)

        U = ExposureControl[EXPOSURE_A].voltage;
        I = ExposureControl[EXPOSURE_A].current;
        F = ExposureControl[EXPOSURE_A].brightness;


        printf("Current: %f, Voltage: %f, Brightness Factor: %f\n", I, U, F);
        // If values for I, U, F are valid
        if (I != 0 && U != 0 && F != 0) {
            // Predict current power using the prediction function
            current_power = power_prediction(I, U, F);

            printf("Predicted Power: %f\n", current_power);
            // If first iteration, set previous_power to current_power
            if (previous_power == 0) {
                previous_power = current_power;
            }

            // Call the function to adjust the LED based on current power and previous power
            adjust_LED(&I, current_power, previous_power);
