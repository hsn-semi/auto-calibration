...
//HSN/////////////////////////////
double global_current_power = 0.0;
double *ptr_cur=&global_current_power;
//HSN//////////////////////////////
...
//current setter for calibration
//void current_set(){
//
//	static int I;
//
...
//
//	int *ptr_I = &I;
//	*ptr_I = ExposureControl[port].currentset;
//
//
//
//
...
////////////////////////////////////////////////////////////////////////////////////////////////////
double power_prediction(int *ptr_I, float *ptr_U, float *ptr_F){
   	double a[7] = {-7.825630201308065e-07, 0.0030517434906565787, 5.976644715772184e-06, -0.00038240819470248145, 0.012802766175777788, -1.272209529111967e-06, -0.07647777294577619};
...
int my_main() {
       ...
       float U , F;  // Initialize current, voltage, and photocurrent
       int I;
       static double previous_power=0.0;
       static double current_power=0.0;
       int *ptr_I = &I;
       float *ptr_U = &U;
       float *ptr_F = &F;
//HSN//////////
//       double *ptr_cur = &current_power;
//HSN/////////
       double *ptr_pre = &previous_power;
       char strt[20];
       char strTemp[100];
       unsigned char port = 0; // port for setting the current
       	           // Read and decode I, U, F (UART reading logic)
       	           *ptr_U = ExposureControl[EXPOSURE_A].voltage * 1e-3;
       	//           I = ExposureControl[EXPOSURE_A].current;
       	           *ptr_I = ExposureControl[port].currentset;
       	           *ptr_F = ExposureControl[EXPOSURE_A].brightness;
           ...
           if (*ptr_I > 0 && *ptr_F > 0 && *ptr_U<=14 && *ptr_U>=11) {

        	   if (*ptr_pre == 0){
        		   *ptr_pre= power_prediction(ptr_I, ptr_U, ptr_F);
        	   }
//HSN////////////////
        	   current_power = power_prediction(ptr_I, ptr_U, ptr_F);
        	   *ptr_cur=current_power; //updates globale ptr_cur with current_power
//        	   *ptr_cur = power_prediction(ptr_I, ptr_U, ptr_F);

               // Call the function to adjust the LED based on current power and previous power
        	  adjust_LED(ptr_I, &current_power, ptr_pre, ptr_U, ptr_F);
//            adjust_LED(ptr_I, ptr_cur, ptr_pre, ptr_U, ptr_F);


//HSN///////////////
               // Set the hardware current using the calculated I value
            ExposureSetCurrent (port, (uint16_t)*ptr_I); // pass the current value (I) to the hardware
...
//               char strt[20];
//               itoa(*ptr_cur, strt,10); // Convert the value ptr_cur points to
//               strcat(strTemp, strt); // Append to strTemp
               SendToCommunicationPort(strTemp, strlen(strTemp));
...
void FanInit(void)
{
 	...
 	{
		FanControl.templow = FAN_DEFAULT_TEMPLOW;// in 1/1000�C
 	}
 	FanControl.temphigh = I2cReadEepromInteger(EEPROM_ADDRESS_FAN_TEMP_HIGH);
	if(FanControl.temphigh == 0xFFFFFFFF)
	{
		FanControl.temphigh = FAN_DEFAULT_TEMPHIGH;// in 1/1000�C
	}
...
//	char ch = ' ';
	...
	if(LogActive == 0)
	{
		...
		if(LogActive == 3)//Stepper-Motor, Servo, Exposure
		{
			...
			double current_power=*ptr_cur;

			strcpy(strTemp,"=;");
			sprintf(strt, "%.6f", ptr_cur ? *ptr_cur : 0.0);
			strcat(strTemp, strt);
			strcat(strTemp, ";");
			//			itoa((int)(current_power*1000), strt,10); // Convert the value ptr_cur points to
			//			sprintf(strt, "%.6f", current_power);
			//			strcat(strTemp, strt); // Append to strTemp
			//			strcat(strTemp, ";"); // Add delimiter
...
//					 itoa(ptr_cur, strt, 10);
//					 sprintf(strt, "%.2e", *ptr_cur); //  sprintf instead of itoa
//					 strcat(strTemp, strt);
//					 strcat(strTemp, ";");
//					 itoa((int)(*ptr_cur * 1e3), strt, 10); // Scale and convert *ptr_cur
//					 strcat(strTemp, strt);
//					 strcat(strTemp, ";");
...
// ***       Purpose:  calculates Temperature in 1/1000�C           *** //
